...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $novo_nivel = isset($_POST["nivel"]) ? $_POST["nivel"] : $func_nivel; // para caso campo desativado (disabled)
    ...
    // Garantia de segurança: usuário nível 2 não pode atribuir nível Master (caso queira adicionar isso)
    elseif ($nivel_admin == 2 && $novo_nivel == 3) {
        $msg = "<div class='alert alert-danger'>Você não pode atribuir nível Master!</div>";
    } else {
        $stmt = $sql->prepare("
            UPDATE funcionarios 
...
<body>
            ...
            <div class="col-auto px-sm-2 px-0 bg-dark" id="fund">
                <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100"
                    ...
                    <ul
                        ...
                        <li class="nav-item"><a href="/fws/FWS_ADM/menu_principal/HTML/menu_principal1.php"
                                class="nav-link px-0" id="cor-fonte"><img
                        ...
                        <li><a href="/fws/FWS_ADM/fast_service/HTML/fast_service.php" class="nav-link px-0"
                                id="cor-fonte"><img src="../../menu_principal/IMG/fastservice.png"><span
                        ...
                        <li><a href="/fws/FWS_ADM/menu_financeiro/HTML/menu_financeiro.php" class="nav-link px-0"
                                id="cor-fonte"><img src="../../menu_principal/IMG/financeiro.png"><span
                                    class="ms-1 d-none d-sm-inline">Financeiro</span></a></li>

                        <li><a href="/fws/FWS_ADM/menu_vendas/HTML/menu_venda.php" class="nav-link px-0"
                                id="cor-fonte"><img src="../../menu_principal/IMG/vendaspai.png"><span
                                    class="ms-1 d-none d-sm-inline">Vendas</span></a></li>

                        <li><a href="/fws/FWS_ADM/estoque/HTML/estoque.php" class="nav-link px-0" id="cor-fonte"><img
                                    src="../../menu_principal/IMG/estoque.png"><span
                                    class="ms-1 d-none d-sm-inline">Estoque</span></a></li>

                        <li><a href="/fws/FWS_ADM/produtos/HTML/lista_produtos.php" class="nav-link px-0" id="cor-fonte"><img
                        ...
                        <li><a href="/fws/FWS_ADM/fornecedores/HTML/lista_fornecedores.php" class="nav-link px-0"
                                id="cor-fonte"><img src="../../menu_principal/IMG/fornecedor.png"><span
                                    class="ms-1 d-none d-sm-inline">Fornecedores</span></a></li>

                        <li><a href="/fws/FWS_ADM/funcionarios/HTML/menu_funcionarios.php" class="nav-link px-0 <?php if($pagina=='funcionarios') echo 'active'; ?>"
                                id="cor-fonte"><img src="../../menu_principal/IMG/funcionarios.png"><span
            ...
            <div class="col py-3" id="conteudo-principal">
                <div class="container-box">
                    ...
                    <form method="POST" id="form-editar">
                        ...
                        <input type="text" class="form-control" name="nome" value="<?= htmlspecialchars($func_nome) ?>"
                            required>

                        <label class="form-label mt-3">Email:</label>
                        <input type="email" class="form-control" name="email" value="<?= htmlspecialchars($func_email) ?>"
                            required>
                        ...
                        <?php if ($func_id == $id_admin): ?>
                        <input type="hidden" name="nivel" value="<?= htmlspecialchars($func_nivel) ?>">
                        <?php endif; ?>

                        <button type="submit" class="btn btn-salvar mt-4 w-100" id="btn-salvar" disabled>Salvar Alterações</button>
                    </form>
                </div>
            </div>
    ...
    <script>
        // Seleciona o formulário e o botão salvar
        const form = document.getElementById('form-editar');
        const btnSalvar = document.getElementById('btn-salvar');

        // Guarda os valores iniciais dos inputs para comparação
        ...
        // Função que verifica se algum valor mudou
        function checarAlteracoes() {
            const nomeAtual = form.nome.value;
            const emailAtual = form.email.value;
            const nivelAtual = form.nivel ? form.nivel.value : null;

            if (
                nomeAtual !== valoresIniciais.nome ||
                emailAtual !== valoresIniciais.email ||
                nivelAtual !== valoresIniciais.nivel
            ) {
                btnSalvar.disabled = false; // habilita botão
            } else {
        ...
        form.nome.addEventListener('input', checarAlteracoes);
        form.email.addEventListener('input', checarAlteracoes);
        if (form.nivel) {
            form.nivel.addEventListener('change', checarAlteracoes);
        }
    </script>
